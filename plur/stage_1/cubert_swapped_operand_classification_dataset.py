...
"""Converts the CuBERT Swapped Operand Classification dataset to PLUR."""
...
class CuBertSwappedOperandClassificationDataset(cubert_dataset.CuBertDataset):
  """Converts CuBERT Swapped Operand Classification data to a PLUR dataset.
  ...
  The task is to predict whether a function contains swapped operands in a
  binary operator.
  ...
  def folder_path(self) -> str:
    """As per superclass."""
    return '20200621_Python/swapped_operands_datasets/'

  def dataset_name(self) -> str:
    """As per superclass."""
    return 'cubert_swapped_operand_classification_dataset'
  ...
  Combinations of functions where one use binary operator’s arguments have been
  swapped, to create a buggy example, or left undisturbed, along with a label
  indicating if this bug-injection has occurred. The JSON fields are:

    function: string, the source code of a function as text.

    label: string, one of (“Correct”, “Swapped operands”) indicating if this is
      a buggy or bug-free example.
    ...
    info: string, an unformatted description of how the example was constructed,
      ...
      “original”) or the operand swap has occurred (e.g., “swapped operands of
      not in”).
  """
