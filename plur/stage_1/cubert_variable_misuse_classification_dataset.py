...
"""Converts the CuBERT Variable Misuse Classification dataset to PLUR."""
...
class CuBertVariableMisuseClassificationDataset(cubert_dataset.CuBertDataset):
  """Converts CuBERT Variable Misuse Classification data to a PLUR dataset.
  ...
  The task is to predict whether a function contains a variable misuse bug.
  ...
  def folder_path(self) -> str:
    """As per superclass."""
    return '20200621_Python/variable_misuse_datasets/'

  def dataset_name(self) -> str:
    """As per superclass."""
    return 'cubert_variable_misuse_classification_dataset'
  ...
  Combinations of functions where one use of a variable may have been replaced
  with another variable defined in the same context, along with a label
  indicating if this bug-injection has occurred. The JSON fields are:
    function: string, the source code of a function as text.

    label: string, one of (“Correct”, “Variable misuse”) indicating if this is a
      buggy or bug-free example.
    ...
    info: string, an unformatted description of how the example was constructed,
      ...
      “original”) or the variable substitution that has occurred (e.g.,
      “correct_variable” → “incorrect_variable”).
  """
