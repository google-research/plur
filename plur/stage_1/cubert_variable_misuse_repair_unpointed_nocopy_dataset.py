...
class CuBertVariableMisuseRepairUnpointedNoCopyDataset(
    cubert_variable_misuse_repair_dataset.CuBertVariableMisuseRepairDataset):
  """As per superclass, but does not allow output copying or input pointers."""
  ...
  def __init__(self,
    ...
    super().__init__(
        ...
        allow_output_copy=False,
        *args,
        **kwargs)

  def dataset_name(self) -> str:
    """As per superclass."""
    return 'cubert_variable_misuse_repair_unpointed_nocopy_dataset'

  def dataset_description(self) -> str:
    """As per superclass."""
    return (super().dataset_description() +
            'It confounds all output tokens, to prevent copying from input. '
            'Also, instead of pointers, it uses token to localize errors.')
